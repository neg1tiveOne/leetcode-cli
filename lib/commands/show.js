...
function genFileName(problem, opts) {
  ...
  const fileNameFormat = file.fmt(config.file.show, problem);
  const filePath = fileNameFormat.split('/');
  const fileName = filePath.pop();
  const parentdir = filePath.join('/')

  file.mkdir(path.join(opts.outdir, parentdir))
  ...
  for (let i = 0; ; ++i) {
    const name = path.join(opts.outdir, parentdir, params.join('.').replace(/\.+/g, '.'));
    if (!file.exist(name))
      return name;
    params[1] = i;
  }
